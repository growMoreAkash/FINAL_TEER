...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('/api/getPreviousDay');
    ...
    <>
      <div className='main' style={{ margin: "0px 0px 80px 0px" }}>
        ...
        <div className='main-box'>
          ...
          <div className='box'>
            <h1 >FR (Morning: 10:15 AM)</h1>
            <h1 style={{ marginTop: '10px' }}>{FData.Daydata == null ? "undefined" : FData.Daydata.FR}</h1>
          </div>

          <div className='box'>
            <h1 >SR (Morning: 11:15 AM)</h1>
            <h1 style={{ marginTop: '10px' }}>{FData.Daydata == null ? "undefined" : FData.Daydata.SR}</h1>
          </div>
